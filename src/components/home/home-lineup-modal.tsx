...
export function HomeLineupModal({ lineup }: { lineup: HomeMatchLineup | null }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4">
      ...
      <SurfaceCard className="relative z-10 w-full max-w-[34rem] space-y-4 p-5">
        <button
          type="button"
          aria-label="라인업 닫기"
          onClick={() => setClosed(true)}
          className="absolute right-4 top-4 inline-flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-700"
        >
          <X className="h-4 w-4" />
        </button>
