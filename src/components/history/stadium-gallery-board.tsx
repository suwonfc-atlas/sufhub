...
function getArchiveYear(stadium: Stadium) {
  if (stadium.archive_year) {
    return String(stadium.archive_year);
  }
...
export function StadiumGalleryBoard({ stadiums }: StadiumGalleryBoardProps) {
      ...
      stadiums.flatMap((stadium) =>
        stadium.images
          ...
          .map((src, index) => ({
            src,
            title: stadium.name,
            memo: stadium.description ?? null,
            year: getArchiveYear(stadium),
            key: `${stadium.id}-${index}-${src}`,
          })),
      ),
    [stadiums],
  );
  ...
  const activeImage =
    galleryImages.find((image) => image.key === activeImageKey) ?? null;
